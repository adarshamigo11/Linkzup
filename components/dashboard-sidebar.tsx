    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.isConnected && !linkedinStatus.isConnected) {
          toast.success(`LinkedIn connected successfully! Account: ${data.linkedinName || "LinkedIn Account"}`)
        }
      ...
      <div className="px-2 sm:px-3 pb-3 sm:pb-4">
        <div className="bg-gray-50 rounded-lg p-2 sm:p-3">
          <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
            ...
            <Button variant="ghost" size="sm" className="p-1 sm:p-1.5 hover:bg-gray-200 rounded-lg">
              <Bell className="h-2 w-2 sm:h-3 sm:w-3 text-gray-500" />
            </Button>
          </div>
          ...
          <div className="space-y-1">
            <Button
              variant="ghost"
              className="w-full justify-start gap-1 sm:gap-2 px-1.5 sm:px-2 py-1.5 sm:py-2 text-gray-600 hover:text-gray-900 hover:bg-gray-200 rounded-lg text-xs transition-all duration-200"
              onClick={() => handleNavigation("/dashboard/settings")}
            >
              <Settings className="h-2 w-2 sm:h-3 sm:w-3 flex-shrink-0" />
