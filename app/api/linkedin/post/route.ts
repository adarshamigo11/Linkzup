  ...
  try {
    ...
    if (!registerResponse.ok) {
      ...
      if (registerResponse.status === 429) {
        console.error("🚫 LinkedIn API rate limit exceeded. Waiting before retry...")
        throw new Error("LinkedIn API rate limit exceeded. Please try again in a few minutes.")
      }
...
export async function POST(req: Request) {
  try {
    ...
    if (response.status === 429) {
      console.error("🚫 LinkedIn API rate limit exceeded during posting")
      return NextResponse.json({ 
        error: "LinkedIn API rate limit exceeded. Please try again in a few minutes.",
        details: "Rate limit hit while posting to LinkedIn"
      }, { status: 429 })
    }
    ...
    if (response.ok) {
      ...
      let errorMessage = `LinkedIn posting failed: ${response.status} ${response.statusText}`
      if (response.status === 401) {
        errorMessage = "LinkedIn authentication failed. Please reconnect your LinkedIn account."
      } else if (response.status === 403) {
        errorMessage = "LinkedIn permission denied. Please check your LinkedIn app permissions."
      } else if (response.status === 429) {
        errorMessage = "LinkedIn API rate limit exceeded. Please try again in a few minutes."
      }
